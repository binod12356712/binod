...
export default function ForgotPasswordModal({ closeModal }) {
  ...
  const handleEmailSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://77.37.86.134:3001/api/forgot-password", {
        email,
      });
      setStep(2);
    } catch (error) {
  ...
  const handleCodeSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://77.37.86.134:3001/verify-code",
        {
          email,
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      await axios.post("https://77.37.86.134:3001/reset-password", {
        email,
        newPassword,
      });
