...
const WalletDashboard = () => {
  ...
  useEffect(() => {
    const fetchWalletData = async () => {
      try {
        const response = await axios.get(
          `https://77.37.86.134:3001/api/wallet/${userId}/balances`
        );
    ...
    const fetchCryptos = async () => {
      try {
        const response = await axios.get(
          "https://77.37.86.134:3001/api/cryptos"
        );
  ...
  useEffect(() => {
    if (
      ...
      setUsdtValue(usdValue.toString());
    } else {
      setUsdtValue("");
    }
  ...
  useEffect(() => {
    if (
      ...
      const cryptoPrice = walletData.prices[selectedSymbol].usd;
      const cryptoValue = usdtValue / cryptoPrice;
      setAmount(cryptoValue.toString());
    } else {
      setAmount("");
    }
  }, [usdtValue, selectedSymbol, walletData]);
  ...
  const handleSendSubmit = async (e) => {
    ...
    try {
      await axios.post("https://77.37.86.134:3001/api/send", {
        userId,
  ...
  const handleConvertSubmit = async (e) => {
    ...
    try {
      await axios.post("https://77.37.86.134:3001/api/withdraw", {
        userId,
        symbol: selectedSymbol,
      ...
      try {
        ...
        if (!isNaN(usdAmount) && usdAmount > 0) {
          const response = await axios.post(
            "https://77.37.86.134:3001/api/convert",
            {
              userId,
              fromSymbol: "usd",
  ...
  const handleRechargeSubmit = async (e) => {
    ...
    try {
      await axios.post("https://77.37.86.134:3001/api/deposit", formData);
